...
export default function TicketDetails() {
    ...
    if (ticketInfo.error) {
        return (
            <p 
                className='max-w-2xl mx-auto p-4 bg-white text-red-300 rounded shadow' 
                >{ticketInfo.error}</p>)
    }
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 bg-white rounded shadow">
        ...
        <p><strong>Numéro e-Ticket:</strong> {ticket.numero}</p>
        <p><strong>Status:</strong> {ticket.status}</p>
        <p><strong>Position dans la file:</strong> {ticket.positionDansFile}</p>
        <p><strong>Nombre de personnes devant:</strong> {ticket.nombreDevant}</p>
        <p><strong>Numéro en cours de traitement:</strong> {numTicketEnCours || 'Aucun'}</p>
