...
export default function ClientPage() {
  ...
  const handleServiceChange = (e) => {
    ...
    fetch(
      `http://localhost:8080/api/services/localisations?service=${nomService}`
    )
      .then((res) => res.json())
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-2xl font-bold text-[#b26148] mb-6">
        Générer un Ticket
      </h1>
      ...
      >
        ...
        >
          ...
          {localisations.length > 0
            ...
            : selectedService && (
                <option disabled>Aucune localisation pour ce service</option>
              )}
        </select>
        ...
        <ul className="space-y-2">
          {clients.map((client) => (
            <li key={client.id} className="p-4 bg-white rounded shadow border">
              <p className="text-[#00a8e2] font-semibold">
                {client.prenom} {client.nom}
              </p>
            </li>
          ))}
