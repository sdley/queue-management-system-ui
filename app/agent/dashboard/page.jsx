...
export default function AgentDashboard() {
    ...
    useEffect(() => {
        if (agentId && service && localisation) {
            fetch(`http://localhost:8080/api/agents/${agentId}/dashboard?nomService=${service}&nomLocalisation=${localisation}`)
                .then(res => res.json())
                .then(data => {
    ...
    const handleNextClient = () => {
        ...
        fetch(`http://localhost:8080/api/agents/${agentId}/next?nomService=${service}&nomLocalisation=${localisation}`, {
            method: "POST",
        })
            .then(res => res.json())
            .then(data => {
    ...
    const handlePreviousClient = () => {
        ...
        fetch(`http://localhost:8080/api/agents/${agentId}/previous?nomService=${service}&nomLocalisation=${localisation}`, {
            method: "POST",
        })
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6">
            <h1 className="text-3xl font-bold text-[#b26148] mb-6">Gestion des appels Agent</h1>

            <section className="mb-8">
                <h2 className="text-xl font-semibold">Agent Informations</h2>
                <p><strong>Agent :</strong> {dashboardData?.agent?.prenom} {dashboardData?.agent?.nom}</p>
                <p><strong>Service :</strong> {dashboardData?.service?.nom}</p>
                <p><strong>Localisation :</strong> {dashboardData?.localisation}</p>
            </section>
            ...
            <section className="mb-8">
                ...
                {dashboardData?.currentTicket ? (
                    <>
                        <p><strong>Nom :</strong> {dashboardData?.currentClient?.prenom} {dashboardData?.currentClient?.nom}</p>
                        <p><strong>Numéro de Ticket :</strong> {dashboardData?.currentTicket?.numero}</p>
                        <p><strong>Status :</strong> {dashboardData?.currentTicket?.status}</p>
                    </>
                ) : (
            ...
            <div className="flex space-x-4">
                <button
                    ...
                    ${loadingPrevBtn ? 'bg-gray-300 text-black cursor-not-allowed' : 'bg-gray-200 hover:bg-gray-300 cursor-pointer'}`}
                >
                ...
                <button
                    ...
                    ${loadingNextBtn ? 'bg-blue-300 cursor-not-allowed' : 'bg-[#00a8e2] hover:bg-[#008dc1] cursor-pointer'}`}
                >
